...
if ! command -v uv &> /dev/null; then
    ...
    # Add uv to PATH if it was just installed (handles sudo case where it goes to /root/.local/bin)
    export PATH="$HOME/.local/bin:$PATH"
    # Also check root's local bin if running as sudo
    if [ -d "/root/.local/bin" ] && [ "$(id -u)" = "0" ]; then
