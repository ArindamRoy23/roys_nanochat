...
def test_causal_self_attention_prefill_no_ve(small_config):
    ...
    assert not has_ve(
        layer_idx,
        small_config.n_layer), "Layer 0 should not have VE for this test"
...
def test_causal_self_attention_prefill_with_ve(small_config):
    ...
    layer_idx = 1
    assert has_ve(layer_idx,
                  small_config.n_layer), "Layer 1 should have VE for this test"
...
def test_causal_self_attention_decode_with_cache(small_config):
    ...
    # Create KV cache
    kv_cache = MockKVCache(B, T_cache, small_config.n_layer,
                           small_config.n_kv_head, head_dim)
    ...
    cos = cos_full[:, cache_pos:cache_pos +
                   T_new, :, :]  # Slice to current position
...
def test_causal_self_attention_shapes_consistency(small_config):
    ...
    for layer_idx in range(small_config.n_layer):
        ...
        x = torch.randn(B, T, small_config.n_embd)
        ve = torch.randn(B, 1, small_config.n_kv_head, head_dim) if has_ve(
            layer_idx, small_config.n_layer) else None
        cos, sin = create_rope_cos_sin(T, head_dim)
        cos_sin = (cos, sin)
        window_size = (-1, -1)
        kv_cache = None
...
def test_block(small_config):
    block = Block(small_config, 0)
    x = torch.randn(1, 16, small_config.n_embd)
    # ve = torch.randn(1, 1, small_config.n_kv_head,
    #                  small_config.n_embd // small_config.n_head)
    ve = None
    cos, sin = create_rope_cos_sin(16,
                                   small_config.n_embd // small_config.n_head)
    cos_sin = (cos, sin)
    window_size = (-1, -1)
    kv_cache = None
...
if __name__ == "__main__":
    config = GPTConfig()
    small_config = GPTConfig(sequence_len=32,
                             n_layer=2,
                             n_head=4,
                             n_kv_head=2,
                             n_embd=128)
